...
> **AD-027 — Crag maintains a fork of Chimera's cports (`CragLinux/cports`, branch `master` — the fork delta was merged onto upstream master and the `astro` branch retired per [AD-029](01-architecture.md#ad-029); moved from `aka-mj/astro-cports` per AD-029), Harbormaster-pinned by the committed `.harbormaster.lock`. Crag's packages and fixes are ordinary in-fork commits; a scheduled update-report tracks currency.** *(Accepted; supersedes AD-001)*
>
...
- **The fork** (Chimera's BSD license retained verbatim in `cports/COPYING.md`; provenance and the fork rationale in `cports/README.md`) contains the cbuild engine, Chimera's ~5000 templates, and Crag's delta on top: cross-build/toolchain fixes upstream had not taken (`main/llvm`, `main/glib`, …), new packages Crag needs (`rauc`, `libubootenv`, `basu`), and downstream overrides (`chrony`, `openssh`, `readline`). Each is a `main/<pkg>:` commit that explains itself. `hm sync --locked` materializes the pinned fork into `cports/`.
- **Crag-owned templates** are enumerated in `build/cports-owned.list` (regenerated from the fork delta by `build/lib/gen-owned-list.sh` after every re-pin), classified `new` (no Chimera equivalent — always built from source) or `mod` (we differ — built from source only when installed). This list replaces the old "derive from patch paths + `astro-cports` collection" mechanism.
- **Changing a template** is now a normal commit to the fork (`main/<pkg>: …`), tested via the Crag CI gates, following `cports/CONTRIBUTING.md` (which — unlike upstream — welcomes AI-assisted contributions gated on ownership + verification + honesty). A `build/patches/cports/` quilt patch or an `astro-cports/` shadow still works as a *local, uncommitted* escape hatch for experiments (the build applies any it finds and resets afterward), but the durable home for a change is a fork commit.
- **Currency** is Crag's responsibility now (the fork's cost): `./build/crag-update-report.sh` sweeps the tree with cbuild's own `update-check` and reports stale packages, prioritized by shipped/owned; a scheduled CI job (`.github/workflows/update-report.yml`) publishes it so re-pin/update work can be planned. Re-pins stay deliberate lock-file diffs (AD-023).
...
Consuming pinned templates has a second dividend: for packages Crag does **not** touch, Chimera's own signed binaries are byte-equivalent in intent to what a from-source build of the pinned template produces — so dev and PR builds need not pay the from-source tax (the LLVM template alone is ~1.5 h) to validate Crag changes.

- **Policy**: dev/PR builds may consume Chimera's official binary repository (`https://repo.chimera-linux.org/current/main`, apk appends `/<arch>`) for unmodified packages. **Release and the nightly source-bootstrap canary remain full-source under Crag keys** — no foreign binary ever enters a release artifact.
- **Selection**: per-invocation `--packages-mode=binary|source`, defaulting to the variant's `[packages].mode` (dev variants say `binary`; absent = `source`). In binary mode the `packages` stage builds from source only: the fork's `new` packages and `boards/common/source-packages.list` (always), plus any `mod` fork template the image actually installs (`build/cports-owned.list`, AD-027). The `rootfs` stage then installs from the local Crag repo *and* Chimera's repo.
- **Precedence** (apk-tools 3.x solver: newest version wins; on equal versions, the earlier-listed repository wins): the local repo is listed first, and manifest entries present in the local repo are installed with exact version pins, so an Crag-built or patched package deterministically beats Chimera's. A newer Chimera version of a *non-manifest* dependency can still be selected — which is exactly what the skew guard reports.
...
- **armv7 exception**: cports carries an armv7 build profile (`armv7-chimera-linux-musleabihf`), but Chimera publishes **no armv7 binary repository** (chimera-repo-main excludes it) — binary packages-mode is therefore unavailable for armv7, and armv7 builds always run in source mode regardless of variant. Mitigation: CI persists and publishes Crag's own built armv7 apk repo, which later builds consume as a warm cache (Crag keys, so the foreign-binary and skew concerns below don't arise). armv7 is Chimera's newest architecture; expect some upstream templates to need arch-gate loosening, carried via the shadow-template mechanism above.
...
- **ccache**: all cbuild source builds (both modes) run with cbuild's transparent ccache support enabled (`ccache = yes` under `[build]` in `cports/etc/config.ini`, written idempotently by the packages stage; opt out with `CRAG_CCACHE=0`). The cache persists at `cports/cbuild_cache/ccache` inside the workspace mount, alongside the §4 cache volumes.

> **AD-023 — cports tracking cadence: re-pin freely during development; each Crag release locks its pin.** *(Accepted)*
...
## 2. cbuild primer (Crag-flavored)

What Crag relies on, so readers don't need the full upstream docs:
...
- **Cross-compilation is transparent**: `./cbuild -a aarch64 pkg main/foo` — same profiles for native and cross. Crag's build container always cross-builds from x86_64 to the target arch (including "cross" x86_64→x86_64 for uniformity of the pipeline).
- **Bootstrap**: `./cbuild bootstrap` builds the bldroot from binary packages. Policy: **day-to-day and PR builds are binary-bootstrapped from Crag's own apk repo** (first from Chimera's repo until Crag's exists); **a source-bootstrap (4-stage, musl-host) canary runs nightly in CI** so the from-source story stays honest without taxing every build.
...
> **AD-002 — cbuild's own in-bldroot LLVM toolchain builds every distro package. The prototype's standalone `build-toolchain.sh` toolchain is repurposed as the Crag app SDK.** *(Recommended)*

The prototype conflated these; Crag separates them:
...
5. The SDK also pins and ships nothing Zig-related — Zig is its own self-contained cross toolchain; the build container pins the Zig version for cragd ([06-config-api.md §3](06-config-api.md)).
...
Single entrypoint: **`crag`** (in `build/`, evolving the prototype's `build.sh` + `build-inner.sh`):

```
crag build  <board> [variant] [--external <tree>…] [--step <stage>…]
crag run    <board> [variant] [--snapshot|--fresh|--detach]
crag deploy <app> --to <host|qemu> [--pkg|--watch]   # dev sideload (AD-026, 08 §6)
crag test   <suite> <board>       # boot-smoke | update | api
crag keys   init-dev
crag clean  [--all|--stage <s>]
```
...
- `SOURCE_DATE_EPOCH` = commit timestamp of the Crag tree, exported through cbuild and mksquashfs (`-mkfs-time`, `-all-time`), sfdisk (no random disk GUIDs — derived from build id), and grubenv/env images.
