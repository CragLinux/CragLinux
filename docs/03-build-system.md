...
### Binary consumption for dev builds (AD-001 consequence)

Pin-and-overlay has a second dividend: for packages Astro does **not** touch, Chimera's own signed binaries are byte-equivalent in intent to what a from-source build of the pinned template produces — so dev and PR builds need not pay the from-source tax (the LLVM template alone is ~1.5 h) to validate Astro changes.
...
- **Selection**: per-invocation `--packages-mode=binary|source`, defaulting to the variant's `[packages].mode` (dev variants say `binary`; absent = `source`). In binary mode the `packages` stage builds from source only: every template in `astro-cports/`, every template shadowed/patched via `build/patches/cports/` (derived from the patch paths), and anything listed in `boards/common/source-packages.list`. The `rootfs` stage then installs from the local Astro repo *and* Chimera's repo.
