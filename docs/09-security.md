...
| Local unprivileged process (compromised app) | **yes** | RO rootfs; per-app users; astrod socket group gating; no arbitrary init control via API; `/etc` write policy |
...
| RAUC dev CA | dev/CI bundle trust | `keys/dev/rauc-ca*` in-repo, CN `ASTRO DEV — DO NOT SHIP` | — | regenerate at will |
...
- **Privilege separation** ([02 §7](02-base-system.md)): astrod unprivileged; per-app users; the closed `astrod-helper` list for root operations (reboot/time/hostname) via dinit oneshots, not setuid binaries — there are **zero setuid binaries** on a prod image (doas is dev-only).
- **D-Bus policy files we author** (in `astro-base-net`/`astro-base-update`): grant `astrod` user access to `net.connman.iwd` and `de.pengutronix.rauc` interfaces explicitly; default-deny for everyone else; team apps have no system-bus access unless their tree ships a policy (discouraged; the HTTP API is the contract).
...
- **Downgrade protection** — AD-021 ([05 §6](05-updates.md)): astrod-enforced monotonic versions, signed-metadata-only comparison, explicit forced-downgrade audit trail.
