...
- **Rejected: busybox initramfs** — Crag has no busybox, and importing one only for an initramfs contradicts the fixed-core principle.
...
|---|---|
| `crag-<board>-<version>.img.zst` | full GPT disk image, **both slots populated identically**, `data` partition minimal (grows on first boot); flashable with `dd`/bmaptool |
| `crag-<board>-<version>.raucb` | RAUC verity bundle updating `boot` + `rootfs` slot group ([05-updates.md](05-updates.md)) |
| `crag-<board>-<version>.qcow2` | QEMU-native conversion of the .img (QEMU boards) |
| `crag-sdk-<arch>-<version>.tar.zst` | app SDK: cross clang wrappers + sysroot generated from this image's package set ([03-build-system.md §3](03-build-system.md)) |
| `manifest.json` | versions (crag, kernel, cports pin, external trees), artifact SHA512s, RAUC compatible string, apk world list |

### Image-stage implementation spec (new code — the prototype stub)

Prior art: chimera-live's `mkimage.sh`/`mkpart.sh`. Crag's implementation (in `build/stages/image`):
...
`crag run <board> [variant]` (evolution of the prototype's run scripts):
...
```
crag build qemu-x86_64 prod                # build image + bundle, version N
crag run qemu-x86_64 --detach              # boots slot A
crag test update qemu-x86_64 \
     --bundle out/…/crag-qemu-x86_64-N+1.raucb
# the test harness, via cragd API on the forwarded port:
