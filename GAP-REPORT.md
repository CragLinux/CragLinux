# Crag M0 Phase 3 — Containerized Build Gap Report

Date: 2026-07-16 · Board/variant: **qemu-aarch64 / dev** · Host: Fedora (24 cores), rootless podman 5.8.4
Entry point: `./build/crag-build.sh qemu-aarch64 dev --step=<stage>` (crag-builder container, `--privileged --userns=keep-id`, repo mounted at `/workspace`).

> **Bottom line: the pipeline now produces a from-source Crag/Chimera rootfs that boots in QEMU to a dinit login prompt** (`qemu-aarch64-development login:`), with udevd, dhcpcd and turnstiled running — via direct kernel boot on a single rw ext4 disk (the M0 model; A/B, squashfs and real bootloader are M1+). Everything in the image was built from the pinned cports checkout — no foreign binary packages.
...
| 10a | QEMU boot — kernel smoke | **PASS** | boot→userspace 2.8 s | Fully-local artifacts: clang/ThinLTO 6.12.95 Image + freestanding static init built with the Crag SDK toolchain, 16 MB ext4, `qemu-system-aarch64 -M virt` inside the container. Kernel boots, PL011 console works, virtio-blk root mounts (`root=/dev/vda`), init executes. Log: `build/state/logs/07-qemu-smoke.log`. |
...
> Crag-touched templates built) and source (removed `libedit-*.apk` from
...
- Suggested fix: resolve the closure via apk against the repo index and `cbuild pkg` anything missing (or use `cbuild`'s bulk facilities) — belongs in the `packages` stage rework of the `crag` CLI (docs/03 §5).
...
3. **Packages-stage closure resolution** (§3.4) so a green build is reproducible from `crag build` alone, plus signed-index verification instead of `--allow-untrusted`.
...
6. **`crag` CLI stage contract** — this run drove `crag-build.sh --step=...` + hand-run cbuild; stage contracts with cache keys (docs/03 §5) would have avoided most manual glue.
...
8. ~~**Boot-smoke test stage** — the QEMU tests in §1 rows 10a/10b should become `crag test boot-smoke <board>` (junit output per docs/03 §5), with a login-prompt assertion and timeout instead of hand-run timeouts.~~ **M1 close-out (2026-07-18): DONE** — `build/test-boot-smoke.sh <board> <variant>`: self-containerizing, boots the full image via `run-qemu.sh --image --scratch=+1G` (scratch overlay keeps the artifact pristine and exercises /data growth), polls serial and exits at verdict (~35-40 s), asserts boot-success + login + zero `[FAILED]` + growth-ran, junit XML in `build/state/test-results/`. All six board/variant combos pass (MIGRATION-NOTES §13).
...
13. **M2 (2026-07-19): AD-020 gate is ON and passing on all three QEMU boards** (`build/test-update-rollback.sh`; MIGRATION-NOTES §14) — full install→flip→mark-good plus poisoned-bundle automatic rollback (U-Boot counter and GRUB try paths both exercised). Wiring it plus boot-smoke into an actual CI workflow (AD-024 thin wrappers) remains open with item 6 (crag CLI stage contracts).
14. **cbuild cross-sysroot world residue** (M2): `-bootstrap` makedeps stay pinned in the cross sysroot world and later conflict with their real packages (`glib-bootstrap` vs `glib`); workaround is deleting `cports/bldroot/usr/<triple>`. Upstream-shaped fix: the cross dep-install path should rewrite the sysroot world like the bldroot path does.
