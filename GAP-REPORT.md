...
13. **M2 (2026-07-19): AD-020 gate is ON and passing on all three QEMU boards** (`build/test-update-rollback.sh`; MIGRATION-NOTES §14) — full install→flip→mark-good plus poisoned-bundle automatic rollback (U-Boot counter and GRUB try paths both exercised). Wiring it plus boot-smoke into an actual CI workflow (AD-024 thin wrappers) remains open with item 6 (astro CLI stage contracts).
