#!/bin/sh
#
# Mount the bootenv partition (vfat) at /run/astro/bootenv so userspace
...
mkdir -p /run/astro/bootenv
if ! mountpoint -q /run/astro/bootenv; then
    ...
    mount -t vfat -o rw,noatime,fmask=0177,dmask=0077 \
        "$bootenv_dev" /run/astro/bootenv || exit 1
fi
