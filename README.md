...
Astro is a complete, working appliance OS for IoT devices and gateways — not a build-system kit. The core is fixed and curated, derived from [Chimera Linux](https://chimera-linux.org): **musl** libc, **FreeBSD userland** (chimerautils), **dinit** init, an all-**LLVM/Clang** toolchain, and **apk-tools v3** packaging. On top of that base, Astro adds what embedded products actually need out of the box:

- **A/B atomic updates** via [RAUC](https://rauc.io) — signed verity bundles, streaming installs, automatic rollback on failed boots.
- **astrod** — a local REST API (think Onics Squid.link) for WiFi/Ethernet config, update control, and system basics; AP-mode captive-portal provisioning included.
- **External trees** — teams ship their apps as real packages with supervised services, pinned against an Astro release, without ever forking the distro.
...
Status: **design phase**. The design supersedes the `clang-cross` prototype, whose toolchain and build machinery Astro imports.
...
| [01 — Architecture](docs/01-architecture.md) | runtime + build architecture, invariants, **decision index (AD-001…AD-025)** |
...
| [06 — Config API](docs/06-config-api.md) | astrod (Zig): architecture, REST conventions, endpoint catalog, auth |
