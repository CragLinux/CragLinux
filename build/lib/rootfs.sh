...
#   source — install everything from the local Crag-built repo, signatures
...
#            Chimera package can never shadow an Crag-built/patched one.
...
create_rootfs() {
    ...
    # CRAG_LOCAL_REPO overrides the local repo base (e.g. a filtered CI copy).
    local pkg_repo_base="${CRAG_LOCAL_REPO:-${PROJECT_ROOT}/cports/packages/main}"
...
# Source packages-mode install: everything comes from the local Crag-built
...
# Binary packages-mode install: local Crag repo (first, wins version ties)
...
create_rootfs_binary() {
    ...
    # Repository list: local Crag repo first — on equal versions apk-tools 3.x
    # prefers the lowest-indexed repository (solver "prefer lowest available
    # repository" tiebreak), so Crag-built packages win over identical
...
#                           §12) — mounted at /run/crag/bootenv by the
...
generate_rauc_config() {
    ...
    {
        echo "# Generated by the Crag rootfs stage (docs/05 §2) — do not edit on-device;"
        ...
        if [ "$backend" = "grub" ]; then
            ...
            echo "grubenv=/run/crag/bootenv/grubenv"
        fi
        echo "statusfile=/data/.crag/rauc.status"
    ...
    # ./build/crag-keys.sh init-dev if missing.
    ...
    if [ "$backend" = "uboot" ]; then
        ...
        {
            echo "# Generated by the Crag rootfs stage. libubootenv config: the U-Boot"
            echo "# environment is a FAT file on the bootenv partition (AD-009 deviation,"
            echo "# MIGRATION-NOTES §12), mounted at /run/crag/bootenv by bootenv-mount."
            echo "/run/crag/bootenv/uboot.env 0x0000 ${env_size}"
...
##############################################################################
# Baked cragd defaults (docs/06 §2, docs/07 §3; M3)
#
# /etc/crag/crag-defaults.json is the IMAGE's contribution to the
# desired-state store: the firstboot oneshot copies it to
# /data/config/crag.json on the first boot of a data lifetime, and
# cragd migrates forward from there. Values come from the board TOML
# [api] section (AD-025: lan_exposure defaults off). Shaped here (jq
# exists in the build container; the image has no jq).
##############################################################################

# Stamp image identity into os-release: cragd's system info prefers
# CRAG_BOARD/CRAG_VARIANT/CRAG_RELEASE over the generic keys, so
# GET /system reports the real board/variant instead of "unknown".
# CRAG_RELEASE mirrors the image/bundle version (CRAG_VERSION, the same
...
# actually served). The Crag document therefore lives in the common
...
stamp_os_release() {
    ...
    sed -i '/^CRAG_/d' "$osr"
    {
        echo "CRAG_BOARD=${board}"
        echo "CRAG_VARIANT=${variant}"
        echo "CRAG_RELEASE=${CRAG_VERSION:-0.0.0-dev}"
    } >> "$osr"
    log_info "os-release stamped: CRAG_BOARD=${board} CRAG_VARIANT=${variant} CRAG_RELEASE=${CRAG_VERSION:-0.0.0-dev}"
    ...
    # \l tty line.
    local issue_text="Crag ${CRAG_VERSION:-0.0.0-dev} (\\n) (\\l) — kernel \\r"
    ...
    log_info "issue banner stamped: Crag ${CRAG_VERSION:-0.0.0-dev}"
}

# Baked network defaults (docs/07 §2 rendering model; M3 phase 3).
#
# 1. /etc/crag/dhcpcd-fallback.conf — the config dhcpcd runs on BEFORE
#    cragd has ever rendered one: tmpfiles pre-creates
#    /run/crag/net/dhcpcd.conf as a symlink here (see the dhcpcd shadow
...
#    minus the resolv.conf hook, which is cragd's job now.
# 2. /etc/resolv.conf -> /run/crag-resolv/resolv.conf — cragd renders
#    the
...
#    Crag target; keep the two in sync.
bake_network_defaults() {
    local rootfs_dir="$1"

    mkdir -p "${rootfs_dir}/etc/crag"
    cat > "${rootfs_dir}/etc/crag/dhcpcd-fallback.conf" << 'EOF'
# Crag dhcpcd FALLBACK config (baked at image build; read-only).
#
# Served through the /run/crag/net/dhcpcd.conf bootstrap symlink until
# cragd's first render replaces it (docs/07 §2; see the dhcpcd dinit
...
# and does not exist before cragd has rendered a profile anyway.
allowinterfaces eth*
# DNS is cragd's: the lease hook exports DHCP-learned resolvers to
# /run/crag/net/leases/ and cragd alone writes the resolv target
# (/run/crag-resolv/resolv.conf).
nohook resolv.conf
# Group cragd may connect to /run/dhcpcd/sock so unprivileged cragd
...
controlgroup cragd
EOF
    log_info "network defaults baked: /etc/crag/dhcpcd-fallback.conf"

    # /run/crag-resolv, NOT /run/crag: the API-socket dir is 0750
    # cragd:crag-api and every user's libc resolver must read this
    # file (chronyd as _chrony found it unreadable live — M3 phase 4).
    ln -sfn /run/crag-resolv/resolv.conf "${rootfs_dir}/etc/resolv.conf"
    log_info "network defaults baked: /etc/resolv.conf -> /run/crag-resolv/resolv.conf"
}

# Time defaults (docs/07 §6, M3 phase 4).
#
# 1. /etc/crag/build-epoch — the image's build timestamp as decimal
...
#    clock up to max(build-epoch, /data/.crag/last-known-time), and
#    cragd re-checks at every startup + gates its own https installs
#    on synced-or-past-floor (cragd/src/timekeep.zig).
# 2. /etc/crag/chrony.conf — the minimal config the chronyd dinit
...
#    directive) when it is set — without it cragd's adjtimex-based
...
bake_time_defaults() {
    ...
    mkdir -p "${rootfs_dir}/etc/crag"
    printf '%s\n' "$epoch" > "${rootfs_dir}/etc/crag/build-epoch"

    cat > "${rootfs_dir}/etc/crag/chrony.conf" << 'EOF'
# Crag minimal chrony config (baked at image build; docs/07 §6).
...
# cragd's time.synced (adjtimex, docs/07 §6); chronyd only touches the
# flag when rtcsync is enabled. Harmless without an RTC driver.
rtcsync
EOF
    log_info "time defaults baked: /etc/crag/build-epoch=${epoch} + /etc/crag/chrony.conf"
}

generate_crag_defaults() {
    local rootfs_dir="$1"
    local board_config_json="$2"
    ...
    }' > "${rootfs_dir}/etc/crag/crag-defaults.json"

    log_info "cragd defaults baked: /etc/crag/crag-defaults.json"
}
