#!/usr/bin/env python3
"""Crag cports update-currency report.
...
path, whether the package is Crag-maintained (part of the fork delta vs
the fork point) and whether Crag installs it in any image (so security
currency on shipped packages can be prioritized over the rest of the tree).

Runs inside the crag-builder container (cbuild + network live there);
build/crag-update-report.sh is the host entry point.

Usage (in-container, cwd = repo root):
    python3 build/lib/update_report.py [--out-dir DIR] [--jobs N] [--scope SCOPE]

  --scope crag   only the Crag-maintained templates (fork delta) — fast
...
def crag_maintained() -> set[str]:
    """Templates that differ from the fork point — the Crag delta we own."""
    out = subprocess.run(
...
def shipped_packages() -> set[str]:
    """Package names Crag installs in any image (base + every board/variant).
...
def main() -> int:
    ...
    ap.add_argument("--scope", choices=("crag", "shipped", "all"),
                    default="crag")
    args = ap.parse_args()

    maintained = crag_maintained()
    ...
    md = [f"# cports update report ({args.scope})", "",
          ...
          "| Package | Current | Latest | Shipped | Crag-owned | Template |",
          "|---|---|---|---|---|---|"]
    for u in updates:
