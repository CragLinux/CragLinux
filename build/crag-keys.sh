...
# Astro Linux - key management (docs/05 §6, docs/09 §4)
#
# Subcommands:
...
#                                   AD-020 test harness and `astro deploy`
...
if [ "$FORCE" = true ] || [ ! -f "$CA_CERT" ] || [ ! -f "$CA_KEY" ] || \
   [ ! -f "$SIGN_CERT" ] || [ ! -f "$SIGN_KEY" ]; then
    ...
    openssl req -new -x509 -key "$CA_KEY" -out "$CA_CERT" -days 3650 \
        -subj "/O=TierOne Software/OU=ASTRO DEV - DO NOT SHIP/CN=Astro Dev RAUC CA" \
    ...
    openssl req -new -key "$SIGN_KEY" -out "$TMP/signing.csr" \
        -subj "/O=TierOne Software/OU=ASTRO DEV - DO NOT SHIP/CN=Astro Dev Bundle Signer"
...
if [ ! -f "$SSH_KEY" ] || [ "$FORCE" = true ]; then
    ...
    ssh-keygen -q -t ed25519 -N "" -C "astro-dev-test-DO-NOT-SHIP" -f "$SSH_KEY"
fi
